...
RESOURCES = {
    ...
    "prod": {
        ...
        "sk": "https://raw.githubusercontent.com/tapis-project/openapi-security/prod/SkAPI.yaml",
        ...
        "jobs": "https://raw.githubusercontent.com/tapis-project/openapi-jobs/prod/JobsAPI.yaml",
    ...
    "staging": {
        ...
        "sk": "https://raw.githubusercontent.com/tapis-project/openapi-security/staging/SkAPI.yaml",
        ...
        "pods": "https://raw.githubusercontent.com/tapis-project/pods_service/prod/docs/openapi_v3-pods.yml",
        "jobs": "https://raw.githubusercontent.com/tapis-project/openapi-jobs/staging/JobsAPI.yaml",
    ...
    "dev": {
        ...
        "sk": "https://raw.githubusercontent.com/tapis-project/openapi-security/dev/SkAPI.yaml",
        ...
        "pods": "https://raw.githubusercontent.com/tapis-project/pods_service/prod/docs/openapi_v3-pods.yml",
        "jobs": "https://raw.githubusercontent.com/tapis-project/openapi-jobs/dev/JobsAPI.yaml",
...
def _get_specs(
    resources: Resources, spec_dir: str = None, download_latest_specs: bool = False
):
    """
    ...
    download_and_pickle_spec_dicts(
        resources, spec_dir=spec_dir, download_latest_specs=download_latest_specs
    )
...
def download_and_pickle_spec_dicts(
    resources: Resources, spec_dir: str, download_latest_specs: bool
) -> None:
    """
...
def _thread_download_spec_dict(resource_info: ResourceInfo) -> None:
    ...
    if response.status_code == 200:
        try:
            ...
            print(
                f'Got exception when test creating spec for "{resource_name}" '
        ...
        try:
            ...
            print(
                f'Got exception when derefrencing spec for "{resource_name}" '
        ...
        raise KeyError(
            f'Error getting "{resource_name}" resource. URL: "{resource_url}".'
    ...
    for resource_name, url in resources.items():
        if "local:" in url:
            ...
            try:
                ...
                print(
                    f'Error reading local "{resource_name}" resource. '
        ...
        try:
            ...
            print(
                f"Got exception trying to unpickle spec for "
...
def update_spec_cache(resources: Resources = None, spec_dir: str = None) -> None:
    ...
    spec_dir = get_spec_dir(spec_dir)
    download_and_pickle_spec_dicts(
        resources, spec_dir=spec_dir, download_latest_specs=True
    )
    ...
    if spec_dir:
        ...
        if not os.path.exists(spec_dir):
            try:
                ...
                shutil.copytree(
                    os.path.join(os.path.dirname(__file__), "specs"), spec_dir
                )
            except PermissionError:
                raise PermissionError(
                    f"You do not have permission to create/write"
    ...
    def reload_tenants(self):
        try:
            ...
            raise errors.BaseTapyException(
                f"Unable to retrieve sites and tenants from the Tenants API. e: {e}"
            )
        for t in tenants:
            for s in sites:
...
class Tapis(object):
    ...
    def __init__(
        ...
        config: Union[
            Config, dict
        ] = Config(),  # Provide a default configuration object
        ...
        **kwargs,
        ...
        if not self.access_token and self.jwt and type(self.jwt) == str:
            if jwt == "dummy":
                ...
                logger.info(
                    "Found jwt='dummy', not adding claims to token, this is only expected during tenant-api initialization."
                )
        ...
        if self.resource_set == "master":
            raise errors.TapyClientConfigurationError(
                "the master branch has been removed from tapis-project "
                "repositories; use 'prod' or 'tapipy' instead."
            )
        if not self.resource_set in RESOURCES.keys():
            raise errors.TapyClientConfigurationError(
                f"'resource_set' must be one of {RESOURCES.keys()}, "
        ...
        if self.custom_spec_dict:
            for spec_name, spec_val in self.custom_spec_dict.items():
                if isinstance(spec_name, str) and isinstance(spec_val, str):
                    ...
                    raise KeyError(
                        f"Custom spec should be a dict of key: str and val: str, got {spec_name} and {spec_val}."
                    )
        ...
        if (
            self.custom_spec_dict
            or self.spec_dir
            or self.download_latest_specs
            or not self.resource_set == "tapipy"
        ):
            resource_specs, self.resource_dicts = _get_specs(
                RESOURCES[resource_set],
                spec_dir=self.spec_dir,
                download_latest_specs=self.download_latest_specs,
            )
        else:
        ...
        if type(config) not in [Config, dict]:
            raise TypeError(
                "Tapis Client Config must be an instance of 'Config' or a dictionary"
            )
        ...
        for p in plugins:
            ...
            importlib.import_module(p).on_tapis_client_instantiation(
                self,
                base_url=base_url,
                username=username,
                password=password,
    ...
    def update_spec_cache(self):
        ...
        if not self.resource_set in RESOURCES.keys():
            raise KeyError(
                f"'resource_set' must be one of {RESOURCES.keys()}, not {self.resource_set}."
            )
        if self.custom_spec_dict:
            for spec_name, spec_val in self.custom_spec_dict.items():
                if isinstance(spec_name, str) and isinstance(spec_val, str):
    ...
    def get_user_tokens(self, **kwargs):
        ...
        if not username or not password:
            if self.refresh_token:
                ...
                raise errors.InvalidInputError(
                    "Either a refresh_token or a username and password are required "
                    "for get_tokens()."
                )
        if client_id and client_key:
            auth_header = {
                "Authorization": get_basic_auth_header(client_id, client_key)
            }
        else:
            auth_header = {}
        ...
        tokens = self.authenticator.create_token(
            username=username,
            password=password,
            grant_type="password",
    ...
    def validate_token(self, token):
        ...
        if not token:
            raise errors.BaseTapyException(
                "No Tapis access token found in the request."
            )
        try:
            data = jwt.decode(
        ...
        try:
            public_key_str = self.tenant_cache.get_tenant_config(
                tenant_id=token_tenant_id
            ).public_key
        except errors.BaseTapyException:
            raise errors.TokenInvalidError(
                "Unable to process Tapis token; unexpected tenant_id."
            )
        except KeyError:
            raise errors.TokenInvalidError(
                "Unable to process Tapis token; no public key associated with the "
                "tenant_id."
            )
        try:
    ...
    def add_claims_to_token(self, token):
        ...
        def _expires_in():
            return token_with_claims.expires_at - datetime.datetime.now(
        ...
        token_with_claims.claims = jwt.decode(
            token_str,
            verify=False,
        ...
        token_with_claims.expires_at = datetime.datetime.fromtimestamp(
            token_with_claims.claims["exp"], datetime.timezone.utc
        )
    ...
    def refresh_user_tokens(self):
        ...
        auth_header = {
            "Authorization": get_basic_auth_header(self.client_id, self.client_key)
        }
        ...
        if type(self.refresh_token) == str:
            ...
            try:
                ...
                raise errors.InvalidInputError(
                    "The refresh_token must either be a string or a refresh_token"
    ...
    def set_refresh_token(self, token):
        ...
        try:
            self.refresh_token.claims = self.validate_token(
                self.refresh_token.refresh_token
            )
    ...
    def upload(self, source_file_path, system_id, dest_file_path, **kwargs):
        ...
        if self.get_access_jwt():
            ...
            if datetime.timedelta(seconds=5) > self.access_token.expires_in():
                ...
                try:
                    ...
                    pass
            headers = {
                "X-Tapis-Token": self.get_access_jwt(),
            }
        ...
        r = requests.Request(
            "POST", url, files={"file": open(source_file_path, "rb")}, headers=headers
        ).prepare()
        ...
        if resp.status_code == 400:
            raise errors.BadRequestError(
                msg=error_msg, version=version, request=r, response=resp
            )
        ...
        if (
            hasattr(resp_content_type, "lower")
            and resp_content_type.lower() == "application/json"
        ):
            try:
                json_content = resp.json()
            except Exception as e:
                msg = (
                    f"Requests could not produce JSON from the response even though the content-type was "
            ...
            if result:
                ...
                if _seq_but_not_str(result):
                    if (
                        len([
                            item
                            for item in result
                            if type(item) in TapisResult.PRIMITIVE_TYPES
                        ])
                        > 0
                    ):
                        if debug:
                ...
                try:
                    ...
                    raise errors.InvalidServerResponseError(
                        msg=msg, version=version, request=r, response=resp
                    )
            else:
        ...
        for path_name, path_desc in self.resource_spec.items():
            ...
            for op, op_desc in path_desc.items():
                ...
                if not op_desc["operationId"]:
                    print(
                        f"invalid op_dec for {resource_name}; missing operationId. op_desc: {op_desc}"
                    )
                    continue
                setattr(
                    self,
                    op_desc["operationId"],
                    Operation(
                        self.resource_name, op_desc, path_name, op, self.tapis_client
                    ),
                )
...
class Operation(object):
    ...
    def __init__(self, resource_name, op_desc, path_name, http_method, tapis_client):
        ...
        self.path_parameters = [
            p for p in op_desc.get("parameters", []) if p["in"] == "path"
        ]
        self.query_parameters = [
            p for p in op_desc.get("parameters", []) if p["in"] == "query"
        ]
    ...
    def __call__(self, **kwargs):
        ...
        for param in self.path_parameters:
            ...
            if param.get("required"):
                if param["name"] not in kwargs:
                    raise errors.InvalidInputError(
                        msg=f"{param['name']} is a required argument."
                    )
            ...
            if param.get("required") and not p_val:
                raise errors.InvalidInputError(
                    msg=f"{param['name']} is a required argument and cannot be None."
                )
        ...
        if access_token:
            ...
            if datetime.timedelta(seconds=5) > time_remaining:
                ...
                if (
                    ...
                    and self.operation_id == "refresh_token"
                ) or (
                    self.resource_name == "authenticator"
                    and self.operation_id == "create_token"
                ):
                    pass
                else:
                    try:
        ...
        if "headers" in kwargs:
            logger.warning(
                "the `headers` argument is deprecated and will be removed in a future version; use _tapis_headers instead."
            )
            try:
                headers.update(kwargs.pop("headers", {}))
            except ValueError:
        ...
        if "_x_tapis_tracking_id" in kwargs:
            if tracking_id:
                raise errors.InvalidInputError(
                    msg="The _x_tapis_tracking_id argument and the X-Tapis-Tracking-ID header cannot both be set."
                )
            else:
        ...
        if tracking_id:
            try:
                ...
                if not all(c.isalnum() or c == "_" for c in tracking_namespace):
                    raise errors.InvalidInputError(
                        msg="Error: tracking_namespace contains invalid characters. Alphanumeric + underscores only."
                    )

                # check tracking_unique_identifier is alphanumeric + hyphens
                if not all(c.isalnum() or c == "-" for c in tracking_unique_identifier):
                    raise errors.InvalidInputError(
                        msg="Error: tracking_unique_identifier contains invalid characters. Alphanumeric + hyphens only."
                    )
        ...
        if "content" in self.request_body and hasattr(
            self.request_body["content"], "keys"
        ):
                ...
                if (
                    ...
                    for p_name, p_desc in (
                        ...
                        if p_name in kwargs:
                            ...
                            raise errors.InvalidInputError(
                                msg=f"{p_name} is a required argument."
                            )
            ...
            # x-www-form-urlencoded
                ...
                raise NotImplementedError(
                    f"Content type/s specified have not been implemented. Types: {request_content_types}"
                )
        ...
        if files:
            r = requests.Request(
                http_method, url, params=params, files=files, headers=headers
            ).prepare()
        else:
            r = requests.Request(
                http_method, url, params=params, data=data, headers=headers
            ).prepare()
            ...
            if "X-Tapis-Token" not in r.headers.keys():
                ...
                if kwargs.get("use_basic_auth", True):
                    ...
                    basic_auth_header = requests.auth.HTTPBasicAuth(
                        self.tapis_client.username, self.tapis_client.service_password
                    )
        ...
        # make the request and return the response object -
        try:
            resp = self.tapis_client.requests_session.send(
                r, verify=self.tapis_client.verify
            )
        except Exception as e:
            # todo - handle different types of requests exceptions
            msg = f"Unable to make request to Tapis server. Exception: {e}"
            ...
            if result or result == [] or result == {}:
                ...
                if _seq_but_not_str(result):
                    if (
                        ...
                        if debug:
                            return [
                                TapisResult(**x) for x in result if not x == "self"
                            ], debug_data
...
class TapisResult(object):
    ...
    def __init__(self, *args, **kwargs):
        ...
        for k, v in kwargs.items():
            ...
            # lists
                ...
                if (
                    len([
                        item for item in v if type(item) in TapisResult.PRIMITIVE_TYPES
                    ])
                    > 0
                ):
                    setattr(self, k, v)
                    ...
                    setattr(
                        self,
                        k,
                        [TapisResult(**item) for item in v if not item == "self"],
                    )
    ...
    def __str__(self):
        attrs = "\n".join([
            f"{str(a)}: {getattr(self, a)}"
            for a in dir(self)
            if not a == "get"
            and not a.startswith("__")
            and not a.startswith("PRIMITIVE_TYPES")
        ])
        return f"\n{attrs}"
