...
/// struct for typed errors of method [`list_db_names`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum ListDbNamesError {
    Status401(),
...
pub async fn list_db_names(configuration: &configuration::Configuration, ) -> Result<Vec<String>, Error<ListDbNamesError>> {
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<ListDbNamesError> = serde_json::from_str(&content).ok();
