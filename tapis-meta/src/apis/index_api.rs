...
pub async fn create_index(configuration: &configuration::Configuration, db: &str, collection: &str, index_name: &str, body: Option<serde_json::Value>) -> Result<(), Error<CreateIndexError>> {
    ...
    let uri_str = format!("{}/meta/{db}/{collection}/_indexes/{indexName}", configuration.base_path, db=crate::apis::urlencode(p_path_db), collection=crate::apis::urlencode(p_path_collection), indexName=crate::apis::urlencode(p_path_index_name));
...
pub async fn delete_index(configuration: &configuration::Configuration, db: &str, collection: &str, index_name: &str) -> Result<(), Error<DeleteIndexError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_path_db = db;
    let p_path_collection = collection;
    let p_path_index_name = index_name;
...
pub async fn list_indexes(configuration: &configuration::Configuration, db: &str, collection: &str) -> Result<Vec<serde_json::Value>, Error<ListIndexesError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_path_db = db;
    let p_path_collection = collection;

    let uri_str = format!("{}/meta/{db}/{collection}/_indexes", configuration.base_path, db=crate::apis::urlencode(p_path_db), collection=crate::apis::urlencode(p_path_collection));
