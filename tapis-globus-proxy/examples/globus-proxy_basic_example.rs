use tapis_globus_proxy::TapisGlobusProxy;
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let client = TapisGlobusProxy::new(&base_url, &jwt_token)?;
    println!("Initialized TapisGlobusProxy against {}", client.config().base_path);
