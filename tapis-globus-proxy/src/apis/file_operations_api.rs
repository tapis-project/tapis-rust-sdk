...
pub async fn delete_path(configuration: &configuration::Configuration, client_id: &str, endpoint_id: &str, path: &str, access_token: &str, refresh_token: &str, recurse: Option<bool>) -> Result<models::RespBasic, Error<DeletePathError>> {
    ...
    let uri_str = format!("{}/v3/globus-proxy/ops/{client_id}/{endpoint_id}/{path}", configuration.base_path, client_id=crate::apis::urlencode(p_path_client_id), endpoint_id=crate::apis::urlencode(p_path_endpoint_id), path=crate::apis::urlencode(p_path_path));
...
pub async fn list_files(configuration: &configuration::Configuration, client_id: &str, endpoint_id: &str, path: &str, access_token: &str, refresh_token: &str, limit: Option<i32>, offset: Option<i32>, filter: Option<&str>) -> Result<models::RespFileList, Error<ListFilesError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_path_client_id = client_id;
    let p_path_endpoint_id = endpoint_id;
    let p_path_path = path;
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ContentType::Json => serde_json::from_str(&content).map_err(Error::from),
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::RespFileList`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::RespFileList`")))),
        }
    } else {
        let content = resp.text().await?;
...
pub async fn make_dir(configuration: &configuration::Configuration, client_id: &str, endpoint_id: &str, path: &str, access_token: &str, refresh_token: &str, req_make_dir: Option<models::ReqMakeDir>) -> Result<models::RespBasic, Error<MakeDirError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_path_client_id = client_id;
    let p_path_endpoint_id = endpoint_id;
    let p_path_path = path;
...
pub async fn rename_path(configuration: &configuration::Configuration, client_id: &str, endpoint_id: &str, path: &str, access_token: &str, refresh_token: &str, req_rename: Option<models::ReqRename>) -> Result<models::RespBasic, Error<RenamePathError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_path_client_id = client_id;
    let p_path_endpoint_id = endpoint_id;
    let p_path_path = path;
