# Rust API client for tapis_globus_proxy
...
Put the package under your project folder in a directory named `tapis_globus_proxy` and add the following to `Cargo.toml` under `[dependencies]`:

```
tapis_globus_proxy = { path = "./tapis_globus_proxy" }
```
