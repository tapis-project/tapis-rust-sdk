...
pub async fn cancel_transfer_task(configuration: &configuration::Configuration, transfer_task_id: &str) -> Result<models::StringResponse, Error<CancelTransferTaskError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_path_transfer_task_id = transfer_task_id;

    let uri_str = format!("{}/v3/files/transfers/{transferTaskId}", configuration.base_path, transferTaskId=crate::apis::urlencode(p_path_transfer_task_id));
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::StringResponse`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::StringResponse`")))),
        }
    } else {
        let content = resp.text().await?;
...
pub async fn create_transfer_task(configuration: &configuration::Configuration, req_transfer: models::ReqTransfer) -> Result<models::TransferTaskResponse, Error<CreateTransferTaskError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ContentType::Json => serde_json::from_str(&content).map_err(Error::from),
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::TransferTaskResponse`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::TransferTaskResponse`")))),
        }
    } else {
        let content = resp.text().await?;
...
pub async fn get_recent_transfer_tasks(configuration: &configuration::Configuration, limit: Option<i32>, offset: Option<i32>) -> Result<models::TransferTaskListResponse, Error<GetRecentTransferTasksError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ContentType::Json => serde_json::from_str(&content).map_err(Error::from),
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::TransferTaskListResponse`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::TransferTaskListResponse`")))),
        }
    } else {
        let content = resp.text().await?;
...
pub async fn get_transfer_task(configuration: &configuration::Configuration, transfer_task_id: &str, include_summary: Option<bool>, impersonation_id: Option<&str>) -> Result<models::TransferTaskResponse, Error<GetTransferTaskError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_path_transfer_task_id = transfer_task_id;
...
pub async fn get_transfer_task_details(configuration: &configuration::Configuration, transfer_task_id: &str, impersonation_id: Option<&str>) -> Result<models::TransferTaskResponse, Error<GetTransferTaskDetailsError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_path_transfer_task_id = transfer_task_id;
    let p_query_impersonation_id = impersonation_id;

    let uri_str = format!("{}/v3/files/transfers/{transferTaskId}/details", configuration.base_path, transferTaskId=crate::apis::urlencode(p_path_transfer_task_id));
    let mut req_builder = configuration.client.request(reqwest::Method::GET, &uri_str);
