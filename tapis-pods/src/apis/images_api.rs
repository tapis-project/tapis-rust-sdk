...
pub async fn add_image(configuration: &configuration::Configuration, new_image: models::NewImage) -> Result<models::ImageResponse, Error<AddImageError>> {
...
pub async fn add_images(configuration: &configuration::Configuration, new_image: Vec<models::NewImage>, skip_duplicates: Option<bool>) -> Result<models::ImagesResponse, Error<AddImagesError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_body_new_image = new_image;
...
pub async fn delete_image(configuration: &configuration::Configuration, image_id: &str) -> Result<models::ImageDeleteResponse, Error<DeleteImageError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_path_image_id = image_id;

    let uri_str = format!("{}/pods/images/{image_id}", configuration.base_path, image_id=p_path_image_id.to_string());
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ContentType::Json => serde_json::from_str(&content).map_err(Error::from),
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::ImageDeleteResponse`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::ImageDeleteResponse`")))),
        }
    } else {
        let content = resp.text().await?;
...
pub async fn get_images(configuration: &configuration::Configuration, ) -> Result<models::ImagesResponse, Error<GetImagesError>> {
