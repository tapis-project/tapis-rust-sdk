...
pub async fn create_secret(configuration: &configuration::Configuration, new_secret: models::NewSecret) -> Result<models::SecretResponse, Error<CreateSecretError>> {
...
pub async fn delete_secret(configuration: &configuration::Configuration, secret_id: &str) -> Result<models::SecretDeleteResponse, Error<DeleteSecretError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_path_secret_id = secret_id;

    let uri_str = format!("{}/pods/secrets/{secret_id}", configuration.base_path, secret_id=crate::apis::urlencode(p_path_secret_id));
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ContentType::Json => serde_json::from_str(&content).map_err(Error::from),
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::SecretDeleteResponse`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::SecretDeleteResponse`")))),
        }
    } else {
        let content = resp.text().await?;
...
pub async fn get_secret(configuration: &configuration::Configuration, secret_id: &str) -> Result<models::SecretResponse, Error<GetSecretError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_path_secret_id = secret_id;
...
pub async fn get_secret_value(configuration: &configuration::Configuration, secret_id: &str) -> Result<models::SecretValueResponse, Error<GetSecretValueError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_path_secret_id = secret_id;

    let uri_str = format!("{}/pods/secrets/{secret_id}/value", configuration.base_path, secret_id=crate::apis::urlencode(p_path_secret_id));
    let mut req_builder = configuration.client.request(reqwest::Method::GET, &uri_str);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ContentType::Json => serde_json::from_str(&content).map_err(Error::from),
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::SecretValueResponse`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::SecretValueResponse`")))),
        }
    } else {
        let content = resp.text().await?;
...
pub async fn list_secrets(configuration: &configuration::Configuration, ) -> Result<models::SecretsResponse, Error<ListSecretsError>> {
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ContentType::Json => serde_json::from_str(&content).map_err(Error::from),
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::SecretsResponse`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::SecretsResponse`")))),
        }
    } else {
        let content = resp.text().await?;
...
pub async fn update_secret(configuration: &configuration::Configuration, secret_id: &str, update_secret: models::UpdateSecret) -> Result<models::SecretResponse, Error<UpdateSecretError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_path_secret_id = secret_id;
