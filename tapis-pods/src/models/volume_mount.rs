...
/// VolumeMount : Volume mount configuration for attaching volumes, snapshots, or inline configs to pods.  Object-keyed structure (v3) - mount_path is the key in volume_mounts dict: - type: One of 'tapisvolume', 'tapissnapshot', 'ephemeral', 'pvc' - source_id: Required for tapisvolume, tapissnapshot, pvc. Not used for ephemeral. - sub_path: Optional sub-path within the source to mount (not applicable for ephemeral) - read_only: Whether mount is read-only (default varies by type) - config_content: For ephemeral type, the inline config file content (max 1MB) - config_permissions: For ephemeral type, Unix file permissions (default 0644) - mounted_by: Service-managed field tracking which user mounted this volume (set by API, not user)
...
impl VolumeMount {
    ...
    pub fn new(r#type: Type) -> VolumeMount {
        VolumeMount {
            r#type,
            source_id: None,
            mounted_by: None,
