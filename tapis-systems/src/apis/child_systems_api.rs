...
pub async fn create_child_system(configuration: &configuration::Configuration, parent_id: &str, req_post_child_system: models::ReqPostChildSystem) -> Result<models::RespResourceUrl, Error<CreateChildSystemError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_path_parent_id = parent_id;
    let p_body_req_post_child_system = req_post_child_system;

    let uri_str = format!("{}/v3/systems/{parentId}/createChildSystem", configuration.base_path, parentId=crate::apis::urlencode(p_path_parent_id));
...
pub async fn unlink_children(configuration: &configuration::Configuration, parent_system_id: &str, all: Option<bool>, req_unlink_children: Option<models::ReqUnlinkChildren>) -> Result<models::RespChangeCount, Error<UnlinkChildrenError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_path_parent_system_id = parent_system_id;
    let p_query_all = all;
    let p_body_req_unlink_children = req_unlink_children;

    let uri_str = format!("{}/v3/systems/{parentSystemId}/unlinkChildren", configuration.base_path, parentSystemId=crate::apis::urlencode(p_path_parent_system_id));
    let mut req_builder = configuration.client.request(reqwest::Method::POST, &uri_str);
...
pub async fn unlink_from_parent(configuration: &configuration::Configuration, child_system_id: &str) -> Result<models::RespChangeCount, Error<UnlinkFromParentError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_path_child_system_id = child_system_id;

    let uri_str = format!("{}/v3/systems/{childSystemId}/unlinkFromParent", configuration.base_path, childSystemId=crate::apis::urlencode(p_path_child_system_id));
    let mut req_builder = configuration.client.request(reqwest::Method::POST, &uri_str);
